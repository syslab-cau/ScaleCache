 ...
 */
void __scxfs_delete_from_page_cache(struct page *page, void *shadow)
{
 ...
 * %false otherwise.
 */
bool scxfs_filemap_range_has_page(struct address_space *mapping,
...
void __scxfs_filemap_set_wb_err(struct address_space *mapping, int err)
{
...
extern void scxfs_mem_cgroup_migrate(struct page *oldpage, struct page *newpage);
 ...
 * Return: %0
 */
int scxfs_replace_page_cache_page(struct page *old, struct page *new, gfp_t gfp_mask)
{
	...
	scxfs_mem_cgroup_migrate(old, new);
	if (freepage)
		freepage(old);
	put_page(old);
...
extern int scxfs_mem_cgroup_try_charge(struct page *page, struct mm_struct *mm,
			  gfp_t gfp_mask, struct mem_cgroup **memcgp,
			  bool compound);
extern void scxfs_mem_cgroup_cancel_charge(struct page *page, struct mem_cgroup *memcg,
		bool compound);

extern void scxfs_mem_cgroup_commit_charge(struct page *page, struct mem_cgroup *memcg,
			      bool lrucare, bool compound);

noinline int __scxfs_add_to_page_cache_locked_optimized(struct page *page,
					...
					void **shadowp)
{
	...
	if (!huge) {
		error = scxfs_mem_cgroup_try_charge(page, current->mm,
...
unlock:
	...
	if (!huge)
		scxfs_mem_cgroup_commit_charge(page, memcg, false, false);
...
int scxfs_add_to_page_cache_lru(struct page *page, struct address_space *mapping,
				pgoff_t offset, gfp_t gfp_mask)
{
	void *shadow = NULL;
	int ret;
	...
	ret = __scxfs_add_to_page_cache_locked_optimized(page, mapping, offset,
					 gfp_mask, &shadow);
	if (unlikely(ret))
...
#ifdef CONFIG_NUMA
static struct page *__scxfs_page_cache_alloc(gfp_t gfp)
{
	int n;
	struct page *page;
	...
	if (cpuset_do_page_mem_spread()) {
		...
		do {
			...
			page = __scxfs_alloc_pages_node(n, gfp, 0);
		} while (!page && read_mems_allowed_retry(cpuset_mems_cookie));

		return page;
	}
	return scxfs_alloc_pages(gfp, 0);
}
#else
struct page *__scxfs_page_cache_alloc(gfp_t gfp)
{
	return scxfs_alloc_pages(gfp, 0);
}
#endif
...
void scxfs_wait_on_page_bit(struct page *page, int bit_nr)
{
	wait_queue_head_t *q = page_waitqueue(page);
...
int scxfs_wait_on_page_bit_killable(struct page *page, int bit_nr)
{
	wait_queue_head_t *q = page_waitqueue(page);
 ...
 * dereference @page.
 */
void scxfs_put_and_wait_on_page_locked(struct page *page)
{
	wait_queue_head_t *q;
 ...
 */
void scxfs_add_page_wait_queue(struct page *page, wait_queue_entry_t *waiter)
{
	wait_queue_head_t *q = page_waitqueue(page);
	unsigned long flags;
 ...
 * test the sign bit).
 */
void scxfs_unlock_page(struct page *page)
{
 ...
 * @page: the page
 */
void scxfs_end_page_writeback(struct page *page)
{
	/*
 ...
 * Return: the found page or shadow entry, %NULL if nothing is found.
 */
struct page *scxfs_find_lock_entry(struct address_space *mapping, pgoff_t offset)
{
	struct page *page;
 ...
 */
static struct page *scxfs_pagecache_get_page(struct address_space *mapping, pgoff_t offset,
...
no_page:
	if (!page && (fgp_flags & FGP_CREAT)) {
		...
		page = __scxfs_page_cache_alloc(gfp_mask);
		if (!page)
			return NULL;
		...
		err = scxfs_add_to_page_cache_lru(page, mapping, offset, gfp_mask);
		if (unlikely(err)) {
			put_page(page);
			page = NULL;
			if (err == -EEXIST)
 ...
 */
unsigned scxfs_find_get_entries(struct address_space *mapping,
 ...
 * reached.
 */
unsigned scxfs_find_get_pages_range(struct address_space *mapping, pgoff_t *start,
 ...
 * Return: the number of pages which were found.
 */
unsigned scxfs_find_get_pages_range_tag(struct address_space *mapping, pgoff_t *index,
...
void scxfs_filemap_map_pages(struct vm_fault *vmf,
...
const struct vm_operations_struct scxfs_generic_file_vm_ops = {
	...
	.map_pages	= scxfs_filemap_map_pages,
					...
					pgoff_t index, unsigned flags)
{
	...
	page = scxfs_pagecache_get_page(mapping, index, fgp_flags,
			mapping_gfp_mask(mapping));
