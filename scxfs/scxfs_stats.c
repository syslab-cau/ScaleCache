...
void scxfs_stats_clearall(struct xfsstats __percpu *stats)
{
	...
	scxfs_notice(NULL, "Clearing xfsstats");
...
#define SCXFSSTAT_START_XQMSTAT xfsstats_offset(xs_qm_dqreclaims)
#define SCXFSSTAT_END_XQMSTAT xfsstats_offset(xs_qm_dquot)

static int xqm_proc_show(struct seq_file *m, void *v)
{
	/* maximum; incore; ratio free to inuse; freelist */
	seq_printf(m, "%d\t%d\t%d\t%u\n",
		   0, counter_val(xfsstats.xs_stats, SCXFSSTAT_END_XQMSTAT),
		   0, counter_val(xfsstats.xs_stats, SCXFSSTAT_END_XQMSTAT + 1));
	return 0;
}
...
static int xqmstat_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "qm");
	for (j = SCXFSSTAT_START_XQMSTAT; j < SCXFSSTAT_END_XQMSTAT; j++)
