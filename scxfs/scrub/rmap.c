	...
	struct scxfs_rmap_irec	*irec)
{
	...
	non_inode = SCXFS_RMAP_NON_INODE_OWNER(irec->rm_owner);
	is_bmbt = irec->rm_flags & SCXFS_RMAP_BMBT_BLOCK;
	is_attr = irec->rm_flags & SCXFS_RMAP_ATTR_FORK;
	is_unwritten = irec->rm_flags & SCXFS_RMAP_UNWRITTEN;

	/* If this is shared, must be a data fork extent. */
	error = scxfs_refcount_find_shared(sc->sa.refc_cur, irec->rm_startblock,
...
xchk_rmapbt_xref(
	...
	struct scxfs_rmap_irec	*irec)
{
	scxfs_agblock_t		agbno = irec->rm_startblock;
	scxfs_extlen_t		len = irec->rm_blockcount;

	if (sc->sm->sm_flags & SCXFS_SCRUB_OFLAG_CORRUPT)
		return;

	xchk_xref_is_used_space(sc, agbno, len);
	if (irec->rm_owner == SCXFS_RMAP_OWN_INODES)
		xchk_xref_is_inode_chunk(sc, agbno, len);
	else
		xchk_xref_is_not_inode_chunk(sc, agbno, len);
	if (irec->rm_owner == SCXFS_RMAP_OWN_COW)
	...
	union scxfs_btree_rec	*rec)
{
	...
	if (irec.rm_owner == SCXFS_RMAP_OWN_FS) {
		/*
		 * scxfs_verify_agbno returns false for static fs metadata.
		 * Since that only exists at the start of the AG, validate
		 * that by hand.
		 */
		if (irec.rm_startblock != 0 ||
		    irec.rm_blockcount != SCXFS_AGFL_BLOCK(mp) + 1)
			xchk_btree_set_corrupt(bs->sc, bs->cur, 0);
	} else {
		/*
		...
		if (!scxfs_verify_agbno(mp, agno, irec.rm_startblock) ||
		    !scxfs_verify_agbno(mp, agno, irec.rm_startblock +
	...
	if (!non_inode) {
		...
		if (irec.rm_owner <= SCXFS_RMAP_OWN_MIN ||
	...
	bool				should_have_rmap)
{
	...
	error = scxfs_rmap_record_exists(sc->sa.rmap_cur, bno, len, oinfo,
			&has_rmap);
	...
	scxfs_extlen_t		len)
{
	...
	error = scxfs_rmap_has_record(sc->sa.rmap_cur, bno, len, &has_rmap);
	if (!xchk_should_check_xref(sc, &error, &sc->sa.rmap_cur))
		return;
	if (has_rmap)
		xchk_btree_xref_set_corrupt(sc, sc->sa.rmap_cur, 0);
