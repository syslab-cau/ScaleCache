 ...
 *
 * Traditionally, SCXFS (the kernel driver) did not know how to check or
 * repair on-disk data structures.  That task was left to the scxfs_check
 * and scxfs_repair tools, both of which require taking the filesystem
 * offline for a thorough but time consuming examination.  Online
 * scrub & repair, on the other hand, enables us to check the metadata
 * for obvious errors while carefully stepping around the filesystem's
 * ongoing operations, locking rules, etc.
 *
 * Given that most SCXFS metadata consist of records stored in a btree,
 ...
 * left untreated.  It is possible for SCXFS to continue if an object is
 ...
 * - "scxfs_" are general SCXFS symbols.
 * - "xchk_" are symbols related to metadata checking.
 * - "xrep_" are symbols related to metadata repair.
 * - "scxfs_scrub_" are symbols that tie online fsck to the rest of SCXFS.
 */

/*
 * Scrub probe -- userspace uses this to probe if we're willing to scrub
 * or repair a given mountpoint.  This will be used by scxfs_scrub to
	...
	int			error)
{
	...
	if (sc->tp) {
		if (error == 0 && (sc->sm->sm_flags & SCXFS_SCRUB_IFLAG_REPAIR))
			error = scxfs_trans_commit(sc->tp);
		else
			scxfs_trans_cancel(sc->tp);
		sc->tp = NULL;
	}
	if (sc->ip) {
		if (sc->ilock_flags)
		...
		if (sc->ip != ip_in &&
		    !scxfs_internal_inum(sc->mp, sc->ip->i_ino))
			scxfs_irele(sc->ip);
		sc->ip = NULL;
	}
...
static const struct xchk_meta_ops meta_scrub_ops[] = {
	[SCXFS_SCRUB_TYPE_PROBE] = {	/* ioctl presence test */
		.type	= ST_NONE,
		.setup	= xchk_setup_fs,
		.scrub	= xchk_probe,
		.repair = xrep_probe,
	},
	[SCXFS_SCRUB_TYPE_SB] = {		/* superblock */
		.type	= ST_PERAG,
		.setup	= xchk_setup_fs,
	...
	[SCXFS_SCRUB_TYPE_FINOBT] = {	/* finobt */
		...
		.setup	= xchk_setup_ag_iallocbt,
		.scrub	= xchk_finobt,
		.has	= scxfs_sb_version_hasfinobt,
		.repair	= xrep_notsupported,
	},
	[SCXFS_SCRUB_TYPE_RMAPBT] = {	/* rmapbt */
		...
		.scrub	= xchk_rmapbt,
		.has	= scxfs_sb_version_hasrmapbt,
		.repair	= xrep_notsupported,
	},
	[SCXFS_SCRUB_TYPE_REFCNTBT] = {	/* refcountbt */
		...
		.has	= scxfs_sb_version_hasreflink,
		.repair	= xrep_notsupported,
	},
	[SCXFS_SCRUB_TYPE_INODE] = {	/* inode record */
		.type	= ST_INODE,
		.setup	= xchk_setup_inode,
		.scrub	= xchk_inode,
		.repair	= xrep_notsupported,
	},
	[SCXFS_SCRUB_TYPE_BMBTD] = {	/* inode data fork */
		.type	= ST_INODE,
		.setup	= xchk_setup_inode_bmap,
		.scrub	= xchk_bmap_data,
		.repair	= xrep_notsupported,
	},
	[SCXFS_SCRUB_TYPE_BMBTA] = {	/* inode attr fork */
		.type	= ST_INODE,
		.setup	= xchk_setup_inode_bmap,
		.scrub	= xchk_bmap_attr,
		.repair	= xrep_notsupported,
	},
	[SCXFS_SCRUB_TYPE_BMBTC] = {	/* inode CoW fork */
		.type	= ST_INODE,
		.setup	= xchk_setup_inode_bmap,
		.scrub	= xchk_bmap_cow,
		.repair	= xrep_notsupported,
	...
	[SCXFS_SCRUB_TYPE_XATTR] = {	/* extended attributes */
		.type	= ST_INODE,
		.setup	= xchk_setup_xattr,
		.scrub	= xchk_xattr,
		.repair	= xrep_notsupported,
	},
	[SCXFS_SCRUB_TYPE_SYMLINK] = {	/* symbolic link */
		.type	= ST_INODE,
		.setup	= xchk_setup_symlink,
		.scrub	= xchk_symlink,
		.repair	= xrep_notsupported,
	},
	[SCXFS_SCRUB_TYPE_PARENT] = {	/* parent pointers */
		.type	= ST_INODE,
		.setup	= xchk_setup_parent,
		.scrub	= xchk_parent,
		.repair	= xrep_notsupported,
	},
	[SCXFS_SCRUB_TYPE_RTBITMAP] = {	/* realtime bitmap */
		.type	= ST_FS,
		.setup	= xchk_setup_rt,
		.scrub	= xchk_rtbitmap,
		.has	= scxfs_sb_version_hasrealtime,
		.repair	= xrep_notsupported,
	},
	[SCXFS_SCRUB_TYPE_RTSUM] = {	/* realtime summary */
		.type	= ST_FS,
		.setup	= xchk_setup_rt,
	...
	[SCXFS_SCRUB_TYPE_UQUOTA] = {	/* user quota */
		.type	= ST_FS,
		.setup	= xchk_setup_quota,
		.scrub	= xchk_quota,
		.repair	= xrep_notsupported,
	},
	[SCXFS_SCRUB_TYPE_GQUOTA] = {	/* group quota */
		.type	= ST_FS,
		.setup	= xchk_setup_quota,
		.scrub	= xchk_quota,
		.repair	= xrep_notsupported,
	},
	[SCXFS_SCRUB_TYPE_PQUOTA] = {	/* project quota */
		.type	= ST_FS,
		.setup	= xchk_setup_quota,
		.scrub	= xchk_quota,
		.repair	= xrep_notsupported,
	},
	[SCXFS_SCRUB_TYPE_FSCOUNTERS] = {	/* fs summary counters */
		.type	= ST_FS,
	...
	struct scxfs_scrub_metadata	*sm)
{
	...
	error = -EINVAL;
	/* Check our inputs. */
	sm->sm_flags &= ~SCXFS_SCRUB_FLAGS_OUT;
	if (sm->sm_flags & ~SCXFS_SCRUB_FLAGS_IN)
		goto out;
	...
	if (sm->sm_type >= SCXFS_SCRUB_TYPE_NR)
		goto out;
...
static inline void xchk_postmortem(struct scxfs_scrub *sc)
{
	/*
	...
	struct scxfs_scrub_metadata	*sm)
{
	...
	BUILD_BUG_ON(sizeof(meta_scrub_ops) !=
		(sizeof(struct xchk_meta_ops) * SCXFS_SCRUB_TYPE_NR));
...
retry_op:
	...
	if ((sc.sm->sm_flags & SCXFS_SCRUB_IFLAG_REPAIR) &&
		...
		if (SCXFS_TEST_ERROR(false, mp, SCXFS_ERRTAG_FORCE_SCRUB_REPAIR))
			sc.sm->sm_flags |= SCXFS_SCRUB_OFLAG_CORRUPT;

		needs_fix = (sc.sm->sm_flags & (SCXFS_SCRUB_OFLAG_CORRUPT |
						SCXFS_SCRUB_OFLAG_XCORRUPT |
		 ...
		 */
		if (!needs_fix) {
			sc.sm->sm_flags |= SCXFS_SCRUB_OFLAG_NO_REPAIR_NEEDED;
			goto out_nofix;
		}
...
out:
	...
	if (error == -EFSCORRUPTED || error == -EFSBADCRC) {
		sm->sm_flags |= SCXFS_SCRUB_OFLAG_CORRUPT;
		error = 0;
	}
	return error;
}
