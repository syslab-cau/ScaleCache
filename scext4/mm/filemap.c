...
{
	LF_XA_STATE(xas, &mapping->i_pages, page->index);
	unsigned int nr = 1;
	...
	if (!PageHuge(page)) {
		lf_xas_set_order(&xas, page->index, compound_order(page));
		nr = compound_nr(page);
	}
...
static void page_cache_delete_batch(struct address_space *mapping,
			     struct pagevec *pvec)
{
	LF_XA_STATE(xas, &mapping->i_pages, pvec->pages[0]->index);
	int total_pages = 0;
	int i = 0;
	struct page *page;
 ...
 */
int __scext4_filemap_fdatawrite_range(struct address_space *mapping, loff_t start,
...
static inline int __filemap_fdatawrite(struct address_space *mapping,
	int sync_mode)
{
	return __scext4_filemap_fdatawrite_range(mapping, 0, LLONG_MAX, sync_mode);
}

int scext4_filemap_fdatawrite(struct address_space *mapping)
{
	return __filemap_fdatawrite(mapping, WB_SYNC_ALL);
}
//EXPORT_SYMBOL(filemap_fdatawrite);

int scext4_filemap_fdatawrite_range(struct address_space *mapping, loff_t start,
				loff_t end)
{
	return __scext4_filemap_fdatawrite_range(mapping, start, end, WB_SYNC_ALL);
}
 ...
 * Return: %0 on success, negative error code otherwise.
 */
int scext4_filemap_flush(struct address_space *mapping)
{
...
{
	struct page *page;
	LF_XA_STATE(xas, &mapping->i_pages, start_byte >> PAGE_SHIFT);
				     ...
				     loff_t start_byte, loff_t end_byte)
{
	...
	while (index <= end) {
		...
		nr_pages = scext4_pagevec_lookup_range_tag(&pvec, mapping, &index,
 ...
 */
int scext4_filemap_fdatawait_range(struct address_space *mapping, loff_t start_byte,
			    loff_t end_byte)
{
 ...
 * fsfreeze(8)
 */
int scext4_filemap_fdatawait_range_keep_errors(struct address_space *mapping,
		loff_t start_byte, loff_t end_byte)
{
	__filemap_fdatawait_range(mapping, start_byte, end_byte);
 ...
 */
int scext4_file_fdatawait_range(struct file *file, loff_t start_byte, loff_t end_byte)
{
 ...
 * Return: error status of the address space.
 */
int scext4_filemap_fdatawait_keep_errors(struct address_space *mapping)
{
 ...
 * Return: error status of the address space.
 */
int scext4_filemap_write_and_wait_range(struct address_space *mapping,
...
void __scext4_filemap_set_wb_err(struct address_space *mapping, int err)
{
 ...
 * Return: %0 on success, negative error code otherwise.
 */
int scext4_file_check_and_advance_wb_err(struct file *file)
{
	int err = 0;
 ...
 * Return: %0 on success, negative error code otherwise.
 */
int scext4_file_write_and_wait_range(struct file *file, loff_t lstart, loff_t lend)
{
	int err = 0, err2;
	struct address_space *mapping = file->f_mapping;
					...
					void **shadowp)
{
	...
	do {
		unsigned int order = lf_xa_get_order(xas.xa, xas.xa_index);
		void *entry, *old = NULL;

		if (order > thp_order(page))
			lf_xas_split_alloc(&xas, lf_xa_load(xas.xa, xas.xa_index),
					order, gfp_mask);
		local_irq_disable();
		preempt_disable();
		//xas_lock_irq(&xas);
...
unlock:
		...
		local_irq_enable();
		preempt_enable();
	} while (lf_xas_nomem(&xas, gfp_mask));
...
struct page *__scext4_page_cache_alloc(gfp_t gfp)
{
	...
	if (cpuset_do_page_mem_spread()) {
		...
		do {
			...
			page = __scext4_alloc_pages_node(n, gfp, 0);
		} while (!page && read_mems_allowed_retry(cpuset_mems_cookie));

		return page;
	}
	return scext4_alloc_pages(gfp, 0);
}
#else
struct page *__scext4_page_cache_alloc(gfp_t gfp)
{
...
extern wait_queue_head_t page_wait_table[PAGE_WAIT_TABLE_SIZE] __cacheline_aligned;
...
void scext4_wait_on_page_bit(struct page *page, int bit_nr)
{
	wait_queue_head_t *q = page_waitqueue(page);
...
int scext4_wait_on_page_bit_killable(struct page *page, int bit_nr)
{
	wait_queue_head_t *q = page_waitqueue(page);
 ...
 * dereference @page.
 */
void scext4_put_and_wait_on_page_locked(struct page *page)
{
	wait_queue_head_t *q;
 ...
 */
void scext4_add_page_wait_queue(struct page *page, wait_queue_entry_t *waiter)
{
	wait_queue_head_t *q = page_waitqueue(page);
	unsigned long flags;
 ...
 * test the sign bit).
 */
void scext4_unlock_page(struct page *page)
{
 ...
 * @page: the page
 */
void scext4_end_page_writeback(struct page *page)
{
	/*
 ...
 * flags appropriately
 */
void scext4_page_endio(struct page *page, bool is_write, int err)
{
	if (!is_write) {
		if (!err) {
...
repeat:
	 ...
	 */
	if (unlikely(page != lf_xas_reload(&xas))) {
		put_page(page);
		goto repeat;
	}
...
no_page:
	if (!page && (fgp_flags & FGP_CREAT)) {
		...
		err = scext4_add_to_page_cache_lru(page, mapping, offset, gfp_mask);
		if (unlikely(err)) {
			put_page(page);
			page = NULL;
			if (err == -EEXIST)
			...
			struct page **pages)
{
	...
	rcu_read_lock();
	lf_xas_for_each_marked(&xas, page, end, tag) {
		if (lf_xas_retry(&xas, page))
			continue;
		/*
 ...
 */
static ssize_t scext4_generic_file_buffered_read(struct kiocb *iocb,
...
scext4_generic_file_read_iter(struct kiocb *iocb, struct iov_iter *iter)
{
	...
	retval = scext4_generic_file_buffered_read(iocb, iter, retval);
out:
	return retval;
}
 ...
 */
vm_fault_t scext4_filemap_fault_internal(struct vm_fault *vmf)
{
	int error;
	struct file *file = vmf->vma->vm_file;
	struct file *fpin = NULL;
		...
		pgoff_t start_pgoff, pgoff_t end_pgoff)
{
	...
	LF_XA_STATE(xas, &mapping->i_pages, start_pgoff);
	struct page *page;

	rcu_read_lock();
	lf_xas_for_each(&xas, page, end_pgoff) {
		if (lf_xas_retry(&xas, page))
			continue;
		if (lf_xa_is_value(page))
			goto next;
...
int scext4_generic_file_mmap(struct file * file, struct vm_area_struct * vma)
{
	struct address_space *mapping = file->f_mapping;
	...
	if (!mapping->a_ops->readpage)
		return -ENOEXEC;
	file_accessed(file);
	vma->vm_ops = &scext4_generic_file_vm_ops;
	return 0;
}
 ...
 */
int scext4_generic_file_readonly_mmap(struct file *file, struct vm_area_struct *vma)
{
	if ((vma->vm_flags & VM_SHARED) && (vma->vm_flags & VM_MAYWRITE))
		return -EINVAL;
	return scext4_generic_file_mmap(file, vma);
}
#else
vm_fault_t scext4_filemap_page_mkwrite(struct vm_fault *vmf)
{
 ...
 */
struct page *scext4_read_cache_page(struct address_space *mapping,
				pgoff_t index,
				int (*filler)(void *, struct page *),
				void *data)
{
 ...
 * Return: up to date page on success, ERR_PTR() on failure.
 */
struct page *scext4_read_cache_page_gfp(struct address_space *mapping,
				pgoff_t index,
				gfp_t gfp)
{
 ...
 */
inline ssize_t scext4_generic_write_checks(struct kiocb *iocb, struct iov_iter *from)
{
	struct file *file = iocb->ki_filp;
	struct inode *inode = file->f_mapping->host;
	loff_t count;
 ...
 */
int scext4_generic_remap_checks(struct file *file_in, loff_t pos_in,
 ...
 */
int scext4_generic_file_rw_checks(struct file *file_in, struct file *file_out)
{
 ...
 */
int scext4_generic_copy_file_checks(struct file *file_in, loff_t pos_in,
			     struct file *file_out, loff_t pos_out,
			     size_t *req_count, unsigned int flags)
{
	...
	ret = scext4_generic_file_rw_checks(file_in, file_out);
	if (ret)
		return ret;
...
int scext4_pagecache_write_begin(struct file *file, struct address_space *mapping,
...
int scext4_pagecache_write_end(struct file *file, struct address_space *mapping,
 ...
 */
struct page *scext4_grab_cache_page_write_begin(struct address_space *mapping,
...
ssize_t __scext4_generic_file_write_iter(struct kiocb *iocb, struct iov_iter *from)
{
	...
	if (iocb->ki_flags & IOCB_DIRECT) {
		...
		if (err == 0) {
			...
			written += status;
			scext4_invalidate_mapping_pages(mapping,
						 pos >> PAGE_SHIFT,
						 endbyte >> PAGE_SHIFT);
		} else {
			/*
 ...
 */
int scext4_try_to_release_page(struct page *page, gfp_t gfp_mask)
{
