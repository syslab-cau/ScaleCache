...
extern long scext4_wb_do_writeback_modified(struct bdi_writeback *wb);
 ...
 */
static void scext4_balance_dirty_pages(struct bdi_writeback *wb,
 ...
 */
void scext4_balance_dirty_pages_ratelimited(struct address_space *mapping)
{
	...
	if (unlikely(current->nr_dirtied >= ratelimit))
		scext4_balance_dirty_pages(wb, current->nr_dirtied);
