...
void *lf_xas_load(struct xa_state *xas, bool rewind)
{
	...
	while (lf_xa_is_node(entry)) {
		...
		//LF_XA_NODE_BUG_ON(node, node->refcnt > 0);
		//if (refcnt > 0)
		...
		node = lf_xa_get_node(xas, lf_xa_to_node(entry));
...
void *lf_xas_load_debug(struct xa_state *xas)
{
	struct xa_node *parent = NULL;
	void *entry = lf_xas_start(xas);
	void **slot;
...
static void lf_xas_delete_node(struct xa_state *xas)
{
	...
	for (;;) {
		...
		if (!lf_xa_parent(xas->xa, node))	// Check if node is root node
			break;
		...
		if (__sync_fetch_and_add(&node->parent->refcnt, 0) > 1) {
			...
			LF_XA_NODE_BUG_ON(node, !__sync_bool_compare_and_swap(&node->gc_flag, 1, 0));
			lf_xas_set_xa_node(xas, node);
			break;
		}
		...
		//LF_XA_NODE_BUG_ON(node, node->shift);
		...
		//xa_node_free(node);
		LF_XA_NODE_BUG_ON(node, !__sync_bool_compare_and_swap(&node->del, 0, 1));
...
static void lf_xas_free_nodes(struct xa_state *xas, struct xa_node *top)
{
	...
	//struct xa_node *node = lf_xa_get_node(top);
	...
	for (;;) {
		...
		if (node->shift && lf_xa_is_node(entry)) {
			...
			child = lf_xa_to_node(entry);
			//child = lf_xa_get_node(lf_xa_to_node(entry));
			//if (child == LF_XAS_RESTART) 
			//	goto next;
			//lf_xa_put_node(node);
			node = child;
			offset = 0;
...
static int lf_xas_expand(struct xa_state *xas, void *head)
{
	...
	if (!head) {	// xarray is empty!
		...
		//node = lf_xa_get_node(xas, lf_xa_to_node(head));
	...
	while (max > max_index(head)) { 
		...
		if (!node) {
			//lf_xa_put_node(lf_xa_to_node(head));
			return -ENOMEM;
		}
		node->count = 1;
...
ascend:
	...
	//	lf_xa_put_node(node);	// it is okay to put node here, since xas->xa_node got refcnt
	return shift;
}
...
static void *lf_xas_create(struct xa_state *xas, bool allow_root)
{
	...
	while (shift > order) {
		...
		if (!entry) {
			void *curr;
			struct xa_node *parent = xas->xa_node;
			node = lf_xa_get_node(xas, lf_xas_alloc(xas, shift));
			if (!node)
				break;
			...
			if (curr = __sync_val_compare_and_swap(
					slot, NULL, lf_xa_mk_node(node)) != NULL) {
				//lf_xa_put_node(node);	// TODO: delete from the list!!
				...
				kfree(entry);
				//pr_cont("(%s:%d) ", __func__, __LINE__);
				node = lf_xa_get_node(xas, lf_xa_to_node(curr));
				goto descend;
			}
			if (parent) {
			...
			BUG_ON((unsigned long) lf_xa_to_node(entry) < 100);
			//pr_cont("(%s:%d) ", __func__, __LINE__);
			node = lf_xa_get_node(xas, lf_xa_to_node(entry));
			...
			if (__sync_bool_compare_and_swap(&node->del, 1, 0)) {
				...
				parent = lf_xa_parent(xas->xa, node);
				if (parent) {
					BUG_ON((unsigned long) parent < 100);
...
void lf_xas_create_range(struct xa_state *xas)
{
	...
	for (;;) {
		...
		for (;;) {
			...
			lf_xas_set_xa_node(xas, lf_xa_parent_locked(xas->xa, node));
		...
		int count, int values)
{
	...
	//LF_XA_NODE_BUG_ON(node, (node_count > LF_XA_CHUNK_SIZE) &&
	//			(node_count < (U8_MAX - LF_XA_CHUNK_SIZE)));
	//LF_XA_NODE_BUG_ON(node, (nr_values > LF_XA_CHUNK_SIZE) && 
	//			(nr_values < (U8_MAX - LF_XA_CHUNK_SIZE)));
	lf_xas_update(xas, node);
/*	if (count < 0)
...
void *lf_xas_store(struct xa_state *xas, void *entry)
{
	...
	if (entry) {
		...
		first = lf_xas_load(xas, false);		// if entry is NULL -> no store?
	}
...
*/
		...
		if (entry) {	// insert
			if (offset == max)		// single index entry
				break;
			if (!lf_xa_is_sibling(entry))	// multi index entry
...
void lf_xas_split(struct xa_state *xas, void *entry, unsigned int order)
{
	...
	struct xa_node *node;
	void *curr = lf_xas_load(xas, false);
	int values = 0;
...
void *__lf_xas_prev(struct xa_state *xas)
{
	...
	while (xas->xa_offset == 255) {
		...
		lf_xas_set_xa_node(xas, lf_xa_parent(xas->xa, xas->xa_node));
		if (!xas->xa_node)
			return set_bounds(xas);
	}
	...
	for (;;) {
		...
		lf_xas_set_xa_node(xas, lf_xa_to_node(entry));
		lf_xas_set_offset(xas);
	}
}
...
void *lf_xas_find_conflict(struct xa_state *xas)
{
	...
	if (lf_xas_top(xas->xa_node)) {
		...
		while (lf_xa_is_node(curr)) {
			struct xa_node *node = lf_xa_get_node(xas, lf_xa_to_node(curr));
			while(__sync_fetch_and_add(&node->gc_flag, 0)){
				//wait for other thread	
			}
			if (__sync_fetch_and_add(&node->del, 0)) {
	...
	for (;;) {
		if (xas->xa_node->shift == xas->xa_shift) {
			...
			lf_xas_set_xa_node(xas, lf_xa_parent_locked(xas->xa, xas->xa_node));
			if (!xas->xa_node)
				break;
			continue;
		}
...
void *__lf_xa_erase(struct xarray *xa, unsigned long index)
{
	...
	LF_XA_STATE(xas, xa, index);
	result = lf_xas_result(&xas, lf_xas_store(&xas, NULL));
	lf_xas_clear_xa_node(&xas);
	return result;
}
...
int __lf_xa_insert(struct xarray *xa, unsigned long index, void *entry, gfp_t gfp)
{
	...
	do {
		curr = lf_xas_load(&xas, false);   //check if the node is exist or not?
		if (!curr) {
			lf_xas_store(&xas, entry);
			if (lf_xa_track_free(xa))
				lf_xas_clear_mark(&xas, LF_XA_FREE_MARK);
...
/**
 * __lf_xa_alloc() - Find somewhere to store this entry in the XArray.
 * @xa: XArray.
 * @id: Pointer to ID.
 ...
 */
int __lf_xa_alloc(struct xarray *xa, u32 *id, void *entry,
		...
		struct lf_xa_limit limit, u32 *next, gfp_t gfp)
{
	...
	ret = __lf_xa_alloc(xa, id, entry, limit, gfp);
...
{
	LF_XA_STATE(xas, xa, index);
	void *entry = lf_xas_load(&xas, true);
