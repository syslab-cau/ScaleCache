...
struct scxfs_mru_cache {
	...
	scxfs_mru_cache_free_func_t free_func; /* Function pointer for freeing. */
	struct delayed_work	work;      /* Workqueue data for reaping.   */
	unsigned int		queued;	   /* work has been queued */
	void			*data;
};

static struct workqueue_struct	*scxfs_mru_reap_wq;
...
_scxfs_mru_cache_migrate(
	struct scxfs_mru_cache	*mru,
	unsigned long		now)
{
	unsigned int		grp;
	...
	struct scxfs_mru_cache_elem *elem)
{
	 ...
	 */
	if (!_scxfs_mru_cache_migrate(mru, now)) {
		mru->time_zero = now;
		if (!mru->queued) {
			mru->queued = 1;
			queue_delayed_work(scxfs_mru_reap_wq, &mru->work,
 ...
 */
STATIC void
_scxfs_mru_cache_clear_reap_list(
	struct scxfs_mru_cache	*mru)
		__releases(mru->lock) __acquires(mru->lock)
{
	struct scxfs_mru_cache_elem *elem, *next;
 ...
 */
STATIC void
_scxfs_mru_cache_reap(
	struct work_struct	*work)
{
	struct scxfs_mru_cache	*mru =
		container_of(work, struct scxfs_mru_cache, work.work);
	...
	spin_lock(&mru->lock);
	next = _scxfs_mru_cache_migrate(mru, jiffies);
	_scxfs_mru_cache_clear_reap_list(mru);
	...
	if ((mru->queued > 0)) {
		...
		if (next <= now)
			...
			next -= now;
		queue_delayed_work(scxfs_mru_reap_wq, &mru->work, next);
	}
...
int
scxfs_mru_cache_init(void)
{
	scxfs_mru_reap_wq = alloc_workqueue("scxfs_mru_cache",
				WQ_MEM_RECLAIM|WQ_FREEZABLE, 1);
	if (!scxfs_mru_reap_wq)
		return -ENOMEM;
	return 0;
}

void
scxfs_mru_cache_uninit(void)
{
	destroy_workqueue(scxfs_mru_reap_wq);
}

/*
 * To initialise a struct scxfs_mru_cache pointer, call scxfs_mru_cache_create()
 ...
 */
int
scxfs_mru_cache_create(
	struct scxfs_mru_cache	**mrup,
	void			*data,
	unsigned int		lifetime_ms,
	unsigned int		grp_count,
	scxfs_mru_cache_free_func_t free_func)
{
	struct scxfs_mru_cache	*mru = NULL;
	int			err = 0, grp;
	...
	INIT_DELAYED_WORK(&mru->work, _scxfs_mru_cache_reap);
...
/*
 * Call scxfs_mru_cache_flush() to flush out all cached entries, calling their
 * free functions as they're deleted.  When this function returns, the caller is
 * guaranteed that all the free functions for all the elements have finished
 * executing and the reaper is not running.
 */
static void
scxfs_mru_cache_flush(
	struct scxfs_mru_cache	*mru)
{
	if (!mru || !mru->lists)
		return;
	...
	_scxfs_mru_cache_migrate(mru, jiffies + mru->grp_count * mru->grp_time);
	_scxfs_mru_cache_clear_reap_list(mru);
...
void
scxfs_mru_cache_destroy(
	struct scxfs_mru_cache	*mru)
{
	if (!mru || !mru->lists)
		return;

	scxfs_mru_cache_flush(mru);
...
/*
 * To insert an element, call scxfs_mru_cache_insert() with the data store, the
 * element's key and the client data pointer.  This function returns 0 on
 * success or ENOMEM if memory for the data element couldn't be allocated.
 */
int
scxfs_mru_cache_insert(
	struct scxfs_mru_cache	*mru,
	unsigned long		key,
	struct scxfs_mru_cache_elem *elem)
{
	...
	if (!error)
		_scxfs_mru_cache_list_insert(mru, elem);
	spin_unlock(&mru->lock);
 ...
 * scxfs_mru_cache_remove() with the data store and the element's key.  On success
 * the client data pointer for the removed element is returned, otherwise this
 * function will return a NULL pointer.
 */
struct scxfs_mru_cache_elem *
scxfs_mru_cache_remove(
	struct scxfs_mru_cache	*mru,
	unsigned long		key)
{
	struct scxfs_mru_cache_elem *elem;
...
/*
 * To remove and element and call the free function, call scxfs_mru_cache_delete()
 * with the data store and the element's key.
 */
void
scxfs_mru_cache_delete(
	struct scxfs_mru_cache	*mru,
	unsigned long		key)
{
	struct scxfs_mru_cache_elem *elem;

	elem = scxfs_mru_cache_remove(mru, key);
	if (elem)
		mru->free_func(mru->data, elem);
}

/*
 * To look up an element using its key, call scxfs_mru_cache_lookup() with the
 ...
 * when this function returns.  Call scxfs_mru_cache_done() to release it.  Note
 ...
 * released.  scxfs_mru_cache_done() should NOT be called when this occurs.
 *
 ...
 * scxfs_mru_cache_lookup() or an scxfs_mru_cache_peek(), call scxfs_mru_cache_done()
 * with the data store pointer.
 */
void
scxfs_mru_cache_done(
	struct scxfs_mru_cache	*mru)
		__releases(mru->lock)
{
	spin_unlock(&mru->lock);
