	...
	uint16_t		flags)
{
	...
	fa = scxfs_inode_validate_extsize(sc->mp, be32_to_cpu(dip->di_extsize),
			mode, flags);
	if (fa)
	...
	uint64_t		flags2)
{
	...
	fa = scxfs_inode_validate_cowextsize(sc->mp,
	...
	uint16_t		flags)
{
	...
	if (flags & ~SCXFS_DIFLAG_ANY)
		goto bad;

	/* rt flags require rt device */
	if ((flags & SCXFS_DIFLAG_REALTIME) && !mp->m_rtdev_targp)
		goto bad;

	/* new rt bitmap flag only valid for rbmino */
	if ((flags & SCXFS_DIFLAG_NEWRTBM) && ino != mp->m_sb.sb_rbmino)
		goto bad;

	/* directory-only flags */
	if ((flags & (SCXFS_DIFLAG_RTINHERIT |
		     SCXFS_DIFLAG_EXTSZINHERIT |
		     SCXFS_DIFLAG_PROJINHERIT |
		     SCXFS_DIFLAG_NOSYMLINKS)) &&
	    !S_ISDIR(mode))
		goto bad;

	/* file-only flags */
	if ((flags & (SCXFS_DIFLAG_REALTIME | FS_XFLAG_EXTSIZE)) &&
	    !S_ISREG(mode))
		goto bad;

	/* filestreams and rt make no sense */
	if ((flags & SCXFS_DIFLAG_FILESTREAM) && (flags & SCXFS_DIFLAG_REALTIME))
		goto bad;
	...
	uint64_t		flags2)
{
	...
	if (flags2 & ~SCXFS_DIFLAG2_ANY)
		xchk_ino_set_warning(sc, ino);

	/* reflink flag requires reflink feature */
	if ((flags2 & SCXFS_DIFLAG2_REFLINK) &&
	...
	/* file/dir-only flags */
	if ((flags2 & SCXFS_DIFLAG2_DAX) && !(S_ISREG(mode) || S_ISDIR(mode)))
		goto bad;

	/* file-only flags */
	if ((flags2 & SCXFS_DIFLAG2_REFLINK) && !S_ISREG(mode))
		goto bad;

	/* realtime and reflink make no sense, currently */
	if ((flags & SCXFS_DIFLAG_REALTIME) && (flags2 & SCXFS_DIFLAG2_REFLINK))
		goto bad;

	/* dax and reflink make no sense, currently */
	if ((flags2 & SCXFS_DIFLAG2_DAX) && (flags2 & SCXFS_DIFLAG2_REFLINK))
		goto bad;
	...
	scxfs_ino_t		ino)
{
	...
	case 3:
		...
		if (dip->di_projid_hi != 0 &&
		    !scxfs_sb_version_hasprojid32bit(&mp->m_sb))
			xchk_ino_set_corrupt(sc, ino);
		break;
	default:
		xchk_ino_set_corrupt(sc, ino);
	...
	/*
	 * di_size.  scxfs_dinode_verify checks for things that screw up
	...
	if (S_ISDIR(mode) && (isize == 0 || isize >= SCXFS_DIR2_SPACE_SIZE))
		xchk_ino_set_corrupt(sc, ino);

	/* Symlinks can't be larger than SYMLINK_MAXLEN */
	if (S_ISLNK(mode) && (isize == 0 || isize >= SCXFS_SYMLINK_MAXLEN))
		xchk_ino_set_corrupt(sc, ino);
	...
	} else if (flags & SCXFS_DIFLAG_REALTIME) {
		/*
	...
	fork_recs =  SCXFS_DFORK_DSIZE(dip, mp) / sizeof(struct scxfs_bmbt_rec);
	switch (dip->di_format) {
	case SCXFS_DINODE_FMT_EXTENTS:
	...
	/* di_forkoff */
	if (SCXFS_DFORK_APTR(dip) >= (char *)dip + mp->m_sb.sb_inodesize)
		xchk_ino_set_corrupt(sc, ino);
	if (dip->di_anextents != 0 && dip->di_forkoff == 0)
		xchk_ino_set_corrupt(sc, ino);
	if (dip->di_forkoff == 0 && dip->di_aformat != SCXFS_DINODE_FMT_EXTENTS)
		xchk_ino_set_corrupt(sc, ino);

	/* di_aformat */
	if (dip->di_aformat != SCXFS_DINODE_FMT_LOCAL &&
	    dip->di_aformat != SCXFS_DINODE_FMT_EXTENTS &&
	    dip->di_aformat != SCXFS_DINODE_FMT_BTREE)
		xchk_ino_set_corrupt(sc, ino);

	/* di_anextents */
	nextents = be16_to_cpu(dip->di_anextents);
	fork_recs =  SCXFS_DFORK_ASIZE(dip, mp) / sizeof(struct scxfs_bmbt_rec);
	...
	scxfs_ino_t			ino)
{
	...
	agino = SCXFS_INO_TO_AGINO(sc->mp, ino);

	/*
	 * Try to get the finobt record.  If we can't get it, then we're
	 * in good shape.
	 */
	error = scxfs_inobt_lookup(sc->sa.fino_cur, agino, SCXFS_LOOKUP_LE,
			&has_record);
	if (!xchk_should_check_xref(sc, &error, &sc->sa.fino_cur) ||
	    !has_record)
		return;

	error = scxfs_inobt_get_rec(sc->sa.fino_cur, &rec, &has_record);
	if (!xchk_should_check_xref(sc, &error, &sc->sa.fino_cur) ||
	    !has_record)
		return;
	...
	if (rec.ir_startino > agino ||
	    rec.ir_startino + SCXFS_INODES_PER_CHUNK <= agino)
		return;

	if (rec.ir_free & SCXFS_INOBT_MASK(agino - rec.ir_startino))
	...
	struct scxfs_dinode	*dip)
{
	...
	error = scxfs_bmap_count_blocks(sc->tp, sc->ip, SCXFS_DATA_FORK,
			&nextents, &count);
	...
	error = scxfs_bmap_count_blocks(sc->tp, sc->ip, SCXFS_ATTR_FORK,
			&nextents, &acount);
	if (!xchk_should_check_xref(sc, &error, NULL))
		return;
	...
	struct scxfs_dinode	*dip)
{
	...
	agno = SCXFS_INO_TO_AGNO(sc->mp, ino);
	agbno = SCXFS_INO_TO_AGBNO(sc->mp, ino);
	...
	xchk_xref_is_owned_by(sc, agbno, 1, &SCXFS_RMAP_OINFO_INODES);
	...
	scxfs_ino_t		ino)
{
	...
	error = scxfs_reflink_inode_has_shared_extents(sc->tp, sc->ip,
			&has_shared);
	if (!xchk_xref_process_error(sc, SCXFS_INO_TO_AGNO(mp, ino),
			SCXFS_INO_TO_AGBNO(mp, ino), &error))
		return;
	if (scxfs_is_reflink_inode(sc->ip) && !has_shared)
		xchk_ino_set_preen(sc, ino);
	else if (!scxfs_is_reflink_inode(sc->ip) && has_shared)
		xchk_ino_set_corrupt(sc, ino);
}
...
xchk_inode(
	struct scxfs_scrub	*sc)
{
	struct scxfs_dinode	di;
	int			error = 0;

	/*
	 * If sc->ip is NULL, that means that the setup function called
	 * scxfs_iget to look up the inode.  scxfs_iget returned a EFSCORRUPTED
	...
	scxfs_inode_to_disk(sc->ip, &di, 0);
