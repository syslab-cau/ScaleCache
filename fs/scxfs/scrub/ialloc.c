...
struct xchk_iallocbt {
	...
	scxfs_agino_t		next_startino;

	/* Expected end of the current inode cluster. */
	scxfs_agino_t		next_cluster_ino;
};
	...
	scxfs_agino_t			agino)
{
	...
	if (!(*pcur))
		return;
	error = scxfs_ialloc_has_inode_record(*pcur, agino, agino, &has_irec);
	...
	scxfs_extlen_t			len)
{
	...
	xchk_xref_is_owned_by(sc, agbno, len, &SCXFS_RMAP_OINFO_INODES);
	...
	scxfs_extlen_t			len)
{
	...
	bno = SCXFS_AGINO_TO_AGBNO(mp, agino);
	if (bno + len <= bno ||
	...
	struct scxfs_dinode		*dip)
{
	...
	fsino = SCXFS_AGINO_TO_INO(mp, bs->cur->bc_private.a.agno, agino);
	irec_free = (irec->ir_free & SCXFS_INOBT_MASK(irec_ino));

	if (be16_to_cpu(dip->di_magic) != SCXFS_DINODE_MAGIC ||
	    (dip->di_version >= 3 && be64_to_cpu(dip->di_ino) != fsino)) {
		xchk_btree_set_corrupt(bs->sc, bs->cur, 0);
		goto out;
	}

	error = scxfs_icache_inode_is_allocated(mp, bs->cur->bc_tp, fsino,
			&ino_inuse);
	if (error == -ENODATA) {
	...
	unsigned int			cluster_base)
{
	...
	nr_inodes = min_t(unsigned int, SCXFS_INODES_PER_CHUNK,
			M_IGEO(mp)->inodes_per_cluster);

	/* Map this inode cluster */
	agbno = SCXFS_AGINO_TO_AGBNO(mp, irec->ir_startino + cluster_base);

	/* Compute a bitmask for this cluster that can be used for holemask. */
	for (cluster_index = 0;
	     cluster_index < nr_inodes;
	     cluster_index += SCXFS_INODES_PER_HOLEMASK_BIT)
		cluster_mask |= SCXFS_INOBT_MASK((cluster_base + cluster_index) /
	...
	imap.im_boffset = SCXFS_INO_TO_OFFSET(mp, irec->ir_startino) <<
			mp->m_sb.sb_inodelog;
	...
	trace_xchk_iallocbt_check_cluster(mp, agno, irec->ir_startino,
			...
			SCXFS_INO_TO_OFFSET(mp, irec->ir_startino +
					  cluster_base));
	...
	error = scxfs_imap_to_bp(mp, bs->cur->bc_tp, &imap, &dip, &cluster_bp,
			0, 0);
	...
	for (cluster_index = 0; cluster_index < nr_inodes; cluster_index++) {
		...
		dip = scxfs_buf_offset(cluster_bp, imap.im_boffset);
	...
	scxfs_trans_brelse(bs->cur->bc_tp, cluster_bp);
	return error;
}
	...
	struct scxfs_inobt_rec_incore	*irec)
{
	 ...
	 */
	for (cluster_base = 0;
	     cluster_base < SCXFS_INODES_PER_CHUNK;
...
xchk_iallocbt_rec_alignment(
	...
	struct scxfs_inobt_rec_incore	*irec)
{
	struct scxfs_mount		*mp = bs->sc->mp;
	 ...
	 */
	if (bs->cur->bc_btnum == SCXFS_BTNUM_FINO) {
		unsigned int	imask;

		imask = min_t(unsigned int, SCXFS_INODES_PER_CHUNK,
	...
	if (iabt->next_startino != NULLAGINO) {
		...
		iabt->next_startino += SCXFS_INODES_PER_CHUNK;
	...
	if (igeo->inodes_per_cluster <= SCXFS_INODES_PER_CHUNK)
		return;
	 ...
	 * after this one.
	 */
	iabt->next_startino = irec->ir_startino + SCXFS_INODES_PER_CHUNK;
	...
	union scxfs_btree_rec		*rec)
{
	...
	if (irec.ir_count > SCXFS_INODES_PER_CHUNK ||
	    irec.ir_freecount > SCXFS_INODES_PER_CHUNK)
		xchk_btree_set_corrupt(bs->sc, bs->cur, 0);

	real_freecount = irec.ir_freecount +
			(SCXFS_INODES_PER_CHUNK - irec.ir_count);
	if (real_freecount != xchk_iallocbt_freecount(irec.ir_free))
		xchk_btree_set_corrupt(bs->sc, bs->cur, 0);

	agino = irec.ir_startino;
	/* Record has to be properly aligned within the AG. */
	if (!scxfs_verify_agino(mp, agno, agino) ||
	    !scxfs_verify_agino(mp, agno, agino + SCXFS_INODES_PER_CHUNK - 1)) {
		xchk_btree_set_corrupt(bs->sc, bs->cur, 0);
		goto out;
	}

	xchk_iallocbt_rec_alignment(bs, &irec);
	if (bs->sc->sm->sm_flags & SCXFS_SCRUB_OFLAG_CORRUPT)
		goto out;
	...
	if (!scxfs_inobt_issparse(irec.ir_holemask)) {
		len = SCXFS_B_TO_FSB(mp,
				SCXFS_INODES_PER_CHUNK * mp->m_sb.sb_inodesize);
		if (irec.ir_count != SCXFS_INODES_PER_CHUNK)
			xchk_btree_set_corrupt(bs->sc, bs->cur, 0);
	...
	holecount = 0;
	len = SCXFS_B_TO_FSB(mp,
			SCXFS_INODES_PER_HOLEMASK_BIT * mp->m_sb.sb_inodesize);
	holes = ~scxfs_inobt_irec_to_allocmask(&irec);
	if ((holes & irec.ir_free) != holes ||
	    irec.ir_freecount > irec.ir_count)
		xchk_btree_set_corrupt(bs->sc, bs->cur, 0);

	for (i = 0; i < SCXFS_INOBT_HOLEMASK_BITS; i++) {
		if (holemask & 1)
			holecount += SCXFS_INODES_PER_HOLEMASK_BIT;
		else if (!xchk_iallocbt_chunk(bs, &irec, agino, len))
			break;
		holemask >>= 1;
		agino += SCXFS_INODES_PER_HOLEMASK_BIT;
	}

	if (holecount > SCXFS_INODES_PER_CHUNK ||
	    holecount + irec.ir_count != SCXFS_INODES_PER_CHUNK)
		xchk_btree_set_corrupt(bs->sc, bs->cur, 0);
	...
	int			which)
{
	...
	scxfs_extlen_t		inobt_blocks = 0;
	scxfs_extlen_t		finobt_blocks = 0;
	int			error;

	if (!sc->sa.ino_cur || !sc->sa.rmap_cur ||
	    (scxfs_sb_version_hasfinobt(&sc->mp->m_sb) && !sc->sa.fino_cur) ||
	    xchk_skip_xref(sc->sm))
		return;

	/* Check that we saw as many inobt blocks as the rmap says. */
	error = scxfs_btree_count_blocks(sc->sa.ino_cur, &inobt_blocks);
	if (!xchk_process_error(sc, 0, 0, &error))
		return;

	if (sc->sa.fino_cur) {
		error = scxfs_btree_count_blocks(sc->sa.fino_cur, &finobt_blocks);
		if (!xchk_process_error(sc, 0, 0, &error))
			return;
	}

	error = xchk_count_rmap_ownedby_ag(sc, sc->sa.rmap_cur,
			&SCXFS_RMAP_OINFO_INOBT, &blocks);
	...
	unsigned long long	inodes)
{
	...
	error = xchk_count_rmap_ownedby_ag(sc, sc->sa.rmap_cur,
			&SCXFS_RMAP_OINFO_INODES, &blocks);
	if (!xchk_should_check_xref(sc, &error, &sc->sa.rmap_cur))
		return;
	inode_blocks = SCXFS_B_TO_FSB(sc->mp, inodes * sc->mp->m_sb.sb_inodesize);
	...
	scxfs_btnum_t		which)
{
	...
	cur = which == SCXFS_BTNUM_INO ? sc->sa.ino_cur : sc->sa.fino_cur;
	error = xchk_btree(sc, cur, xchk_iallocbt_rec, &SCXFS_RMAP_OINFO_INOBT,
			&iabt);
	if (error)
		return error;
...
xchk_inobt(
	struct scxfs_scrub	*sc)
{
	return xchk_iallocbt(sc, SCXFS_BTNUM_INO);
}
...
xchk_finobt(
	struct scxfs_scrub	*sc)
{
	return xchk_iallocbt(sc, SCXFS_BTNUM_FINO);
}
	...
	bool			should_have_inodes)
{
	...
	error = scxfs_ialloc_has_inodes_at_extent(*icur, agbno, len, &has_inodes);
