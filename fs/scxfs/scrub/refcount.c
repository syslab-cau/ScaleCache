	...
	void				*priv)
{
	...
	scxfs_agblock_t			rm_last;
	scxfs_agblock_t			rc_last;
	int				error = 0;
	...
	if (refchk->refcount == 1 && rec->rm_owner != SCXFS_RMAP_OWN_COW) {
	...
	struct xchk_refcnt_check	*refchk)
{
	...
	scxfs_agblock_t			next_rbno;
	scxfs_nlink_t			nr;
	scxfs_nlink_t			target_nr;
	...
	scxfs_nlink_t			refcount)
{
	...
	error = scxfs_rmap_query_range(sc->sa.rmap_cur, &low, &high,
	...
	union scxfs_btree_rec	*rec)
{
	...
	scxfs_agblock_t		*cow_blocks = bs->private;
	...
	has_cowflag = (bno & SCXFS_REFC_COW_START);
...
xchk_refcount_xref_rmap(
	struct scxfs_scrub	*sc,
	scxfs_filblks_t		cow_blocks)
{
	scxfs_extlen_t		refcbt_blocks = 0;
	...
	error = scxfs_btree_count_blocks(sc->sa.refc_cur, &refcbt_blocks);
	if (!xchk_btree_process_error(sc, sc->sa.refc_cur, 0, &error))
		return;
	error = xchk_count_rmap_ownedby_ag(sc, sc->sa.rmap_cur,
			&SCXFS_RMAP_OINFO_REFC, &blocks);
	if (!xchk_should_check_xref(sc, &error, &sc->sa.rmap_cur))
		return;
...
xchk_refcountbt(
	struct scxfs_scrub	*sc)
{
	scxfs_agblock_t		cow_blocks = 0;
	int			error;

	error = xchk_btree(sc, sc->sa.refc_cur, xchk_refcountbt_rec,
			&SCXFS_RMAP_OINFO_REFC, &cow_blocks);
	if (error)
		return error;
	...
	scxfs_extlen_t			len)
{
	...
	error = scxfs_refcount_lookup_le(sc->sa.refc_cur,
			agbno + SCXFS_REFC_COW_START, &has_refcount);
	...
	error = scxfs_refcount_get_rec(sc->sa.refc_cur, &rc, &has_refcount);
	if (!xchk_should_check_xref(sc, &error, &sc->sa.refc_cur))
		return;
	if (!has_refcount) {
		xchk_btree_xref_set_corrupt(sc, sc->sa.refc_cur, 0);
	...
	has_cowflag = (rc.rc_startblock & SCXFS_REFC_COW_START);
	...
	scxfs_extlen_t		len)
{
	...
	error = scxfs_refcount_has_record(sc->sa.refc_cur, agbno, len, &shared);
	if (!xchk_should_check_xref(sc, &error, &sc->sa.refc_cur))
		return;
	if (shared)
		xchk_btree_xref_set_corrupt(sc, sc->sa.refc_cur, 0);
