 ...
 */
#undef TRACE_SYSTEM
#define TRACE_SYSTEM scxfs_scrub

#if !defined(_TRACE_SCXFS_SCRUB_TRACE_H) || defined(TRACE_HEADER_MULTI_READ)
#define _TRACE_SCXFS_SCRUB_TRACE_H
...
TRACE_DEFINE_ENUM(SCXFS_SCRUB_TYPE_PROBE);
TRACE_DEFINE_ENUM(SCXFS_SCRUB_TYPE_SB);
TRACE_DEFINE_ENUM(SCXFS_SCRUB_TYPE_AGF);
TRACE_DEFINE_ENUM(SCXFS_SCRUB_TYPE_AGFL);
TRACE_DEFINE_ENUM(SCXFS_SCRUB_TYPE_AGI);
TRACE_DEFINE_ENUM(SCXFS_SCRUB_TYPE_BNOBT);
TRACE_DEFINE_ENUM(SCXFS_SCRUB_TYPE_CNTBT);
TRACE_DEFINE_ENUM(SCXFS_SCRUB_TYPE_INOBT);
TRACE_DEFINE_ENUM(SCXFS_SCRUB_TYPE_FINOBT);
TRACE_DEFINE_ENUM(SCXFS_SCRUB_TYPE_RMAPBT);
TRACE_DEFINE_ENUM(SCXFS_SCRUB_TYPE_REFCNTBT);
TRACE_DEFINE_ENUM(SCXFS_SCRUB_TYPE_INODE);
TRACE_DEFINE_ENUM(SCXFS_SCRUB_TYPE_BMBTD);
TRACE_DEFINE_ENUM(SCXFS_SCRUB_TYPE_BMBTA);
TRACE_DEFINE_ENUM(SCXFS_SCRUB_TYPE_BMBTC);
TRACE_DEFINE_ENUM(SCXFS_SCRUB_TYPE_DIR);
TRACE_DEFINE_ENUM(SCXFS_SCRUB_TYPE_XATTR);
TRACE_DEFINE_ENUM(SCXFS_SCRUB_TYPE_SYMLINK);
TRACE_DEFINE_ENUM(SCXFS_SCRUB_TYPE_PARENT);
TRACE_DEFINE_ENUM(SCXFS_SCRUB_TYPE_RTBITMAP);
TRACE_DEFINE_ENUM(SCXFS_SCRUB_TYPE_RTSUM);
TRACE_DEFINE_ENUM(SCXFS_SCRUB_TYPE_UQUOTA);
TRACE_DEFINE_ENUM(SCXFS_SCRUB_TYPE_GQUOTA);
TRACE_DEFINE_ENUM(SCXFS_SCRUB_TYPE_PQUOTA);
TRACE_DEFINE_ENUM(SCXFS_SCRUB_TYPE_FSCOUNTERS);

#define SCXFS_SCRUB_TYPE_STRINGS \
	{ SCXFS_SCRUB_TYPE_PROBE,		"probe" }, \
	{ SCXFS_SCRUB_TYPE_SB,		"sb" }, \
	{ SCXFS_SCRUB_TYPE_AGF,		"agf" }, \
	{ SCXFS_SCRUB_TYPE_AGFL,		"agfl" }, \
	{ SCXFS_SCRUB_TYPE_AGI,		"agi" }, \
	{ SCXFS_SCRUB_TYPE_BNOBT,		"bnobt" }, \
	{ SCXFS_SCRUB_TYPE_CNTBT,		"cntbt" }, \
	{ SCXFS_SCRUB_TYPE_INOBT,		"inobt" }, \
	{ SCXFS_SCRUB_TYPE_FINOBT,	"finobt" }, \
	{ SCXFS_SCRUB_TYPE_RMAPBT,	"rmapbt" }, \
	{ SCXFS_SCRUB_TYPE_REFCNTBT,	"refcountbt" }, \
	{ SCXFS_SCRUB_TYPE_INODE,		"inode" }, \
	{ SCXFS_SCRUB_TYPE_BMBTD,		"bmapbtd" }, \
	{ SCXFS_SCRUB_TYPE_BMBTA,		"bmapbta" }, \
	{ SCXFS_SCRUB_TYPE_BMBTC,		"bmapbtc" }, \
	{ SCXFS_SCRUB_TYPE_DIR,		"directory" }, \
	{ SCXFS_SCRUB_TYPE_XATTR,		"xattr" }, \
	{ SCXFS_SCRUB_TYPE_SYMLINK,	"symlink" }, \
	{ SCXFS_SCRUB_TYPE_PARENT,	"parent" }, \
	{ SCXFS_SCRUB_TYPE_RTBITMAP,	"rtbitmap" }, \
	{ SCXFS_SCRUB_TYPE_RTSUM,		"rtsummary" }, \
	{ SCXFS_SCRUB_TYPE_UQUOTA,	"usrquota" }, \
	{ SCXFS_SCRUB_TYPE_GQUOTA,	"grpquota" }, \
	{ SCXFS_SCRUB_TYPE_PQUOTA,	"prjquota" }, \
	{ SCXFS_SCRUB_TYPE_FSCOUNTERS,	"fscounters" }

DECLARE_EVENT_CLASS(xchk_class,
	TP_PROTO(struct scxfs_inode *ip, struct scxfs_scrub_metadata *sm,
		 int error),
	TP_ARGS(ip, sm, error),
	TP_STRUCT__entry(
		__field(dev_t, dev)
	...
	TP_printk("dev %d:%d ino 0x%llx type %s agno %u inum %llu gen %u flags 0x%x error %d",
		  ...
		  __entry->ino,
		  __print_symbolic(__entry->type, SCXFS_SCRUB_TYPE_STRINGS),
		  __entry->agno,
		  __entry->inum,
		  __entry->gen,
		  __entry->flags,
...
TRACE_EVENT(xchk_op_error,
	TP_PROTO(struct scxfs_scrub *sc, scxfs_agnumber_t agno,
		 scxfs_agblock_t bno, int error, void *ret_ip),
	TP_ARGS(sc, agno, bno, error, ret_ip),
	TP_STRUCT__entry(
		__field(dev_t, dev)
		__field(unsigned int, type)
		__field(scxfs_agnumber_t, agno)
		__field(scxfs_agblock_t, bno)
		__field(int, error)
		__field(void *, ret_ip)
	),
	TP_fast_assign(
...
TRACE_EVENT(xchk_file_op_error,
	TP_PROTO(struct scxfs_scrub *sc, int whichfork,
		 scxfs_fileoff_t offset, int error, void *ret_ip),
	...
	TP_STRUCT__entry(
		...
		__field(unsigned int, type)
		__field(scxfs_fileoff_t, offset)
		__field(int, error)
		__field(void *, ret_ip)
	),
	TP_fast_assign(
...
DECLARE_EVENT_CLASS(xchk_block_error_class,
	TP_PROTO(struct scxfs_scrub *sc, scxfs_daddr_t daddr, void *ret_ip),
	...
	TP_fast_assign(
		...
		fsbno = SCXFS_DADDR_TO_FSB(sc->mp, daddr);
		agno = SCXFS_FSB_TO_AGNO(sc->mp, fsbno);
		bno = SCXFS_FSB_TO_AGBNO(sc->mp, fsbno);
...
DEFINE_EVENT(xchk_block_error_class, name, \
	TP_PROTO(struct scxfs_scrub *sc, scxfs_daddr_t daddr, \
		 void *ret_ip), \
	TP_ARGS(sc, daddr, ret_ip))
...
DECLARE_EVENT_CLASS(xchk_ino_error_class,
	TP_PROTO(struct scxfs_scrub *sc, scxfs_ino_t ino, void *ret_ip),
...
DEFINE_EVENT(xchk_ino_error_class, name, \
	TP_PROTO(struct scxfs_scrub *sc, scxfs_ino_t ino, \
		 void *ret_ip), \
	TP_ARGS(sc, ino, ret_ip))
...
DECLARE_EVENT_CLASS(xchk_fblock_error_class,
	TP_PROTO(struct scxfs_scrub *sc, int whichfork,
		 scxfs_fileoff_t offset, void *ret_ip),
...
TRACE_EVENT(xchk_incomplete,
	TP_PROTO(struct scxfs_scrub *sc, void *ret_ip),
	TP_ARGS(sc, ret_ip),
	TP_STRUCT__entry(
		__field(dev_t, dev)
		__field(unsigned int, type)
...
TRACE_EVENT(xchk_btree_op_error,
	TP_PROTO(struct scxfs_scrub *sc, struct scxfs_btree_cur *cur,
	...
	TP_fast_assign(
		scxfs_fsblock_t fsbno = xchk_btree_cur_fsbno(cur, level);

		__entry->dev = sc->mp->m_super->s_dev;
		__entry->type = sc->sm->sm_type;
		__entry->btnum = cur->bc_btnum;
		__entry->level = level;
		__entry->agno = SCXFS_FSB_TO_AGNO(cur->bc_mp, fsbno);
		__entry->bno = SCXFS_FSB_TO_AGBNO(cur->bc_mp, fsbno);
...
TRACE_EVENT(xchk_xref_error,
	TP_PROTO(struct scxfs_scrub *sc, int error, void *ret_ip),
...
TRACE_EVENT(xchk_iallocbt_check_cluster,
	TP_PROTO(struct scxfs_mount *mp, scxfs_agnumber_t agno,
		 scxfs_agino_t startino, scxfs_daddr_t map_daddr,
	...
	TP_STRUCT__entry(
		...
		__field(scxfs_daddr_t, map_daddr)
...
TRACE_EVENT(xchk_fscounters_calc,
	TP_PROTO(struct scxfs_mount *mp, uint64_t icount, uint64_t ifree,
...
TRACE_EVENT(xchk_fscounters_within_range,
	TP_PROTO(struct scxfs_mount *mp, uint64_t expected, int64_t curr_value,
		 int64_t old_value),
...
TRACE_EVENT(xrep_calc_ag_resblks,
	TP_PROTO(struct scxfs_mount *mp, scxfs_agnumber_t agno,
		 scxfs_agino_t icount, scxfs_agblock_t aglen, scxfs_agblock_t freelen,
		 scxfs_agblock_t usedlen),
	TP_ARGS(mp, agno, icount, aglen, freelen, usedlen),
	TP_STRUCT__entry(
		__field(dev_t, dev)
		__field(scxfs_agnumber_t, agno)
		__field(scxfs_agino_t, icount)
		__field(scxfs_agblock_t, aglen)
		__field(scxfs_agblock_t, freelen)
		__field(scxfs_agblock_t, usedlen)
	),
	TP_fast_assign(
		__entry->dev = mp->m_super->s_dev;
		__entry->agno = agno;
...
TRACE_EVENT(xrep_calc_ag_resblks_btsize,
	TP_PROTO(struct scxfs_mount *mp, scxfs_agnumber_t agno,
		 scxfs_agblock_t bnobt_sz, scxfs_agblock_t inobt_sz,
		 scxfs_agblock_t rmapbt_sz, scxfs_agblock_t refcbt_sz),
	TP_ARGS(mp, agno, bnobt_sz, inobt_sz, rmapbt_sz, refcbt_sz),
	TP_STRUCT__entry(
		__field(dev_t, dev)
		__field(scxfs_agnumber_t, agno)
		__field(scxfs_agblock_t, bnobt_sz)
		__field(scxfs_agblock_t, inobt_sz)
		__field(scxfs_agblock_t, rmapbt_sz)
		__field(scxfs_agblock_t, refcbt_sz)
	),
	TP_fast_assign(
		__entry->dev = mp->m_super->s_dev;
		__entry->agno = agno;
...
TRACE_EVENT(xrep_reset_counters,
	TP_PROTO(struct scxfs_mount *mp),
	TP_ARGS(mp),
	TP_STRUCT__entry(
		__field(dev_t, dev)
	),
...
TRACE_EVENT(xrep_ialloc_insert,
	TP_PROTO(struct scxfs_mount *mp, scxfs_agnumber_t agno,
		 scxfs_agino_t startino, uint16_t holemask, uint8_t count,
...
#endif /* _TRACE_SCXFS_SCRUB_TRACE_H */
