 ...
 * AGF and AGI scrubbers check them, and a normal scxfs_scrub run checks the
	...
	struct scxfs_scrub	*sc)
{
	...
	for (agno = 0; agno < mp->m_sb.sb_agcount; agno++) {
		...
		error = scxfs_ialloc_read_agi(mp, sc->tp, agno, &agi_bp);
		if (error)
			break;
		error = scxfs_alloc_read_agf(mp, sc->tp, agno, 0, &agf_bp);
		if (error)
			break;
		error = -ENOMEM;
		if (!agf_bp || !agi_bp)
	...
	struct scxfs_inode	*ip)
{
	...
	scxfs_icount_range(sc->mp, &fsc->icount_min, &fsc->icount_max);
	...
	struct scxfs_scrub	*sc)
{
	...
	if (!xchk_process_error(sc, 0, SCXFS_SB_BLOCK(mp), &error))
		return error;
	if (sc->sm->sm_flags & SCXFS_SCRUB_OFLAG_INCOMPLETE)
		return 0;
