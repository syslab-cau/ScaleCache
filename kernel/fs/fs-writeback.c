...
EXPORT_SYMBOL(finish_writeback_work);
...
long wb_check_start_all(struct bdi_writeback *wb)
{
	...
	// approximated the number of inodes (per_cpu inodes)
	nr_pages = get_nr_dirty_pages();
	if (nr_pages) {
		struct wb_writeback_work work = {
...
void (*wb_workfn_in_scext4)(struct work_struct *work) = NULL;
EXPORT_SYMBOL(wb_workfn_in_scext4);
